...
class GameApp {
    ...
    setupSettingsTabs() {
        ...
        document.getElementById('masterVolume').addEventListener('change', (e) => {
            this.updateSoundSettings({ masterVolume: e.target.value });
        });

        document.getElementById('musicVolume').addEventListener('change', (e) => {
            this.updateSoundSettings({ musicVolume: e.target.value });
        });

        document.getElementById('sfxVolume').addEventListener('change', (e) => {
            this.updateSoundSettings({ sfxVolume: e.target.value });
        });

        // Add event listeners for chime settings
        const chimeInputs = document.querySelectorAll('.chime-option input');
        chimeInputs.forEach(input => {
            input.addEventListener('change', (e) => {
                const chimeSettings = {};
                chimeSettings[e.target.id.replace('chime', '').toLowerCase()] = e.target.checked;
                this.updateSoundSettings({ chimes: chimeSettings });
            });
        });

        // Add event listeners for graphics settings
        document.getElementById('graphicsSetting1').addEventListener('change', (e) => {
            this.updateGraphicsSettings({ setting1: e.target.value });
        });

        document.getElementById('graphicsSetting2').addEventListener('change', (e) => {
            this.updateGraphicsSettings({ setting2: e.target.value });
        });
    }
    ...
    setupEventListeners() {
        ...
        //==============================
        // Handle eye in password field
        //==============================

document.querySelectorAll('.eyeBoxIcon svg').forEach(icon => {
    icon.addEventListener('click', function() {
        event.preventDefault();
        const input = this.closest('.combinedTextBox').querySelector('input[type="password"], input[type="text"]');
        if (input) {
            input.type = input.type === 'password' ? 'text' : 'password';
            const eyeOpen = this.closest('.eyeBoxIcon').querySelector('.eye-open');
            const eyeClosed = this.closest('.eyeBoxIcon').querySelector('.eye-closed');
